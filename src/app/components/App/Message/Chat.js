...
const Chat = ({ navigation, salon }) => {
    ...
    useEffect(() => {
        const get_messages = async () => {
            try {
                const response = await GetMessages("chat", "messages", loggedInUserId, salon.id);
                ...
                const formattedMessages = response.map((message) => ({
                    ...
                    user: {
                      _id: message.sender === loggedInUserId ? loggedInUserId : salon.id,
                    },
    ...
    const onSend = useCallback((messages = [], image = null) => {
        if (messages.length > 0) {
            ...
            const send_message = async () => {
                try {
                    const response = await SendMessage("chat", "send_message", new_message);
        ...
        <View style={styles.chatContainer}>
            <View style={styles.chatHeader}>
                ...
                <View style={styles.avatar}>
                    <Image source={{ uri: REQUEST_URL + salon.image }} style={styles.avatarImg} resizeMode="cover" />
                </View>
                <Text style={styles.headerTitle}>{salon.name && title(salon.name)}</Text>
            </View>
            {
